...
if "S110" in cpp_defines:
    softdevice_ver = "s110"
elif "S130" in cpp_defines:
    softdevice_ver = "s130"
elif "S132" in cpp_defines:
    softdevice_ver = "s132"
elif "S113" in cpp_defines:
    softdevice_ver = "s113"
...
if softdevice_ver:
    ...
    ldscript_dir = join(FRAMEWORK_DIR, "cores",
                        ...
                        "toolchain", "armgcc")
    # xxx old: mcu_family = board.get("build.arduino.ldscript", "").split("_")[1]
    mcu_family = board.get("build.arduino.ldscript", "") # xxx
    ldscript_file = "not found" # xxx
    for f in listdir(ldscript_dir):
        if f.endswith(mcu_family) and softdevice_ver in f.lower():
            ldscript_path = join(ldscript_dir, f)
            ldscript_file = f

    # xxx following added by John Maloney for debugging soft devices
    print("")
    print("*** Softdevice:", softdevice_ver)
    print("***   Path:", hex_path)
    print("***   Hex:", softdevice_file)
    print("***   MCU Family:", mcu_family)
    print("***   Loader:", ldscript_file)
    print("***   Defs:", cpp_defines)
    print("")
